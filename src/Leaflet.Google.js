...
// 🍂class Google
// 🍂aka GridLayer.Google
// 🍂extends GridLayer
export const Google = L.GridLayer.extend({
	options: {
	...
	createTile: function (coords, done) {
		...
		for (var i = 0; i < this._imagesPerTile; ++i) {
			const key2 = key + "/" + i,
				imgNode = this._lru && this._lru.get(key2);
			if (imgNode) {
...
L.GridLayer.Google = Google;

// 🍂factory gridLayer.google(options)
// Returns a new `GridLayer.Google` given its options
L.gridLayer.google = function (options) {
	return new L.GridLayer.Google(options);
};
